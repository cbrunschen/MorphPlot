    ...
    while (i != cEnd) {
      ...
      *covered += *(outlineAndHatch(outlineChains, hatchChains, workspace, penRadius, i->hatchAngle(), i->hatchPhase(), penCircle, marks));
      ...
      if (i == cEnd && !thinChains.empty()) {
        ...
        if (steps()) {
          combinedOutlineAndThin->writePng(stepper_->makeName("combined_outline_and_thin.png"));
        }
        ...
        if (steps()) {
          ditheredOnly->writePng(stepper_->makeName("dithered_only.png"));
        }
        dithered.clear();
      ...
      if (dither_) {
        ...
        dithered.addChains(hatchChains);
      }
